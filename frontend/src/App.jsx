    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 flex flex-col font-sans">
      ...
      <div className="flex flex-1 overflow-hidden relative">
            ...
            {navItems.map((item) => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-all ${
                  state.activeTab === item.id ? 'bg-emerald-600/10 text-emerald-400 border border-emerald-500/20' : 'text-slate-400 hover:bg-slate-800'
                }`}
              >
                <item.icon />
        ...
        <main className="flex-1 overflow-y-auto z-10 p-6">
          ...
          {state.activeTab === 'settings' && <AccountSettings user={state.user} onUpdateUser={state.updateUser} onLogout={state.logout} />}
        </main>
      </div>
    </div>
  );
