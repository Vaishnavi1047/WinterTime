      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-lg p-5 shadow-sm">
          ...
          <div className="mt-2 flex items-baseline gap-2">
            <span className="text-3xl font-bold text-slate-100">{actual.toLocaleString()}</span>
            <span className="text-sm text-slate-500">tCO2e</span>
          </div>
        </div>

        {/* Mandated Target Box - NOW DYNAMIC */}
        <div className="bg-slate-800 border border-slate-700 rounded-lg p-5 shadow-sm">
          <p className="text-slate-400 text-sm font-medium uppercase tracking-wider">BEE Mandated Target</p>
          <div className="mt-2 flex items-baseline gap-2">
            <span className="text-3xl font-bold text-slate-100">{target.toLocaleString()}</span>
            <span className="text-sm text-slate-500">tCO2e</span>
          </div>
        </div>
        ...
        <div className={`bg-slate-800 border rounded-lg p-5 shadow-sm transition-colors duration-500 ${status.status === 'COMPLIANT' ? 'border-emerald-500/50' : 'border-red-500/50'
          ...
          <div className="mt-2 flex items-center justify-between">
            ...
            <div className="text-right">
              <span className={`text-sm font-mono font-bold ${status.status === 'COMPLIANT' ? 'text-emerald-500' : 'text-red-500'}`}>
                {status.status === 'COMPLIANT' ? `+${gapValue}` : `-${gapValue}`}
              </span>
              <p className="text-[10px] text-slate-500 uppercase">tCO2e Variance</p>
            </div>
          </div>
          <p className="mt-2 text-xs text-slate-400 leading-relaxed italic border-t border-slate-700/50 pt-2">
            {status.message}
          </p>
        </div>
      </div>
            ...
            <div className="space-y-4">
              ...
              <div className="mt-8 p-3 bg-slate-900 rounded border border-slate-700">
                ...
                <p className="text-xl font-bold text-slate-200">
                  {recentLog.calculatedEmissions?.toLocaleString()} <span className="text-sm font-normal text-emerald-500 ml-1">tCO2e</span>
                </p>
              </div>
            </div>
          ) : (
